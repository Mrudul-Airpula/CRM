...
module.exports.getuserlistwithfilter = async (event) => {
  ...
  let sql = "select txtFirstName from crm.tblusers where '" + filtertype + "' like ' % " + filtervalue + " % ';"
...
module.exports.getuserlist = async (event) => {
  let sql = "select txtFirstName,txtApprovalstatus from crm.tblusers where txtApprovalstatus = 'Pending';"
  let result = await new Promise((resolve) => {
    con.query(sql, function (err, result) {
      if (err) throw err;
      console.log("Result:" + JSON.stringify(result))
...
module.exports.prospectGrowth = async (event) => {
  let sql = "SELECT  COUNT(d.txtConversionType) as count FROM crm.tblleads a JOIN crm.tblleadcampaignmap b ON a.id = b.refLeadId JOIN crm.tblactivity c ON b.id = c.refMapid JOIN crm.tblconversiontype d ON c.refConversionStatus = d.id WHERE d.txtConversionType = 'Prospect';"
  let result = await new Promise((resolve) => {
    con.query(sql, function (err, result) {
      if (err) throw err
      console.log(JSON.stringify(result))
